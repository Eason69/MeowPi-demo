...
void kmnetwork::netStart() {
    network_thread = std::thread([this] { net_receive(); });
}

void kmnetwork::neteJoin() {
    if (network_thread.joinable()) {
        network_thread.join();
    }
}

void kmnetwork::net_receive() {
    // 创建UDP socket
...
void kmnetwork::mouse_automove(int x, int y, int ms){
    // 计算每一步的x和y移动量
...
void kmnetwork::mouse_move_beizer(int x, int y, int ms, int x1, int y1, int x2, int y2){
    Point start = {x1, y1};
    Point control = {x2, y2};
    Point end = {x, y};
...
void kmnetwork::mouse_listen(struct input_event &ev){
    if(is_listen){
        if (ev.type == EV_KEY) {
...
void kmnetwork::keyboard_listen(struct input_event &ev){
    if (!is_listen) {
        return;
    }
    auto it = km_keyboard_map.find(ev.code);
    if (it == km_keyboard_map.end()) {
        return;
    }
    uint8_t key = it->second;
    // 处理控制键
    if (key >= KM_LEFTCONTROL && key <= KM_RIGHT_GUI) {
        if (ev.value == 1) { // button press
            keyboard_report.buttons |= (1 << (key - KM_LEFTCONTROL));
        } else if (ev.value == 0) { // button release
            keyboard_report.buttons &= ~(1 << (key - KM_LEFTCONTROL));
        }
        net_hid_send();
...
void kmnetwork::handleKeyChanges(soft_keyboard_t current_report){
    // 处理控制键（8个按钮）
...
int kmnetwork::findKeyByValue(int value) {
    auto it = reverse_km_keyboard_map.find(value);
    return (it != reverse_km_keyboard_map.end()) ? it->second : value;
}

std::string kmnetwork::exec(const char* cmd) {
...
bool kmnetwork::iskeyboardBlocked(int key){
    return blocked_keyboard.find(key) == blocked_keyboard.end();
}

bool kmnetwork::ismouseBlocked(int key){
