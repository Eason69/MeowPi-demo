...
void catnetwork::net_receive() {
    ...
    encry_key = expandTo16Bytes((int)server_uuid);
    ...
    while (!is_stop && !is_catnet) {
        ...
        if(Configuration::GetInstance().config_encbox) {
            ...
            AESDecrypt((char *)&rx, decryptedData, len);
            // 处理解密后的数据
            std::memcpy(&rx_enc, decryptedData, len);
            net_handle(rx_enc);
            // 清理
...
void catnetwork::net_handle(client_tx data){
    if(data.head.mac == server_uuid) {
        ...
        // 发送响应回客户端
        int err = sendto(sockServerfd, (char *) &data, len, MSG_CONFIRM, (const struct sockaddr *) &addrClient,
                         sizeof(addrClient));
        if (err < 0) {
...
// 解密数据
void catnetwork::AESDecrypt(const char* encryptedData, char* decryptedData, size_t paddedSize) {
    AES_KEY aesKey;

    AES_set_decrypt_key(reinterpret_cast<const unsigned char*>(encry_key), 128, &aesKey);  // 设置 AES-128 密钥
    // 执行解密
    for (size_t i = 0; i < paddedSize; i += AES_BLOCK_SIZE) {
        AES_decrypt(reinterpret_cast<const unsigned char*>(encryptedData) + i,
                    reinterpret_cast<unsigned char*>(decryptedData) + i, &aesKey);
    }

    // 去除填充
    size_t paddingSize = decryptedData[paddedSize - 1];
    size_t dataSize = paddedSize - paddingSize;
    std::memmove(decryptedData, decryptedData, dataSize);  // 移动数据，去掉填充
}

const char* catnetwork::expandTo16Bytes(int mac) {
    static char encry_key[17];  // 16 字节 + 1 字节的终止符
    std::memset(encry_key, 0, sizeof(encry_key));  // 初始化为 0

    // 将 mac 转换为 8 位十六进制字符串
    std::stringstream ss_uuid;
    ss_uuid << std::hex << std::setw(8) << std::setfill('0') << mac;
    std::string str_uuid = ss_uuid.str();

    // 填充到 16 字节
    for (size_t i = 0; i < 16; ++i) {
        encry_key[i] = str_uuid[i % str_uuid.size()];  // 循环填充
    }
