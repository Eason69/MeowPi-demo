...
void Configuration::loadConfig() {
    ...
    // 获取 catbox 组
    config_setting_t *catbox = config_lookup(&cfg, "Catbox");
    if (!catbox) {
        catbox = config_setting_add(config_root_setting(&cfg), "Catbox", CONFIG_TYPE_GROUP);
        config_modified = true;
    }

    // 读取 Catbox.port
    if (config_lookup_int(&cfg, "Catbox.port", &config_catport)) {
        std::cout << "Catbox.port = " << config_catport << std::endl;
    } else {
        std::cout << "未找到 Catbox.port，创建默认值 " << DEFAULT_CATPORT << std::endl;
        config_setting_t *port_setting = config_setting_add(catbox, "port", CONFIG_TYPE_INT);
        config_setting_set_int(port_setting, DEFAULT_CATPORT);
        config_modified = true;
    }
...
void Configuration::save_default_config() {
    ...
    // Catbox settings
    config_setting_t *catbox = config_setting_add(root, "Catbox", CONFIG_TYPE_GROUP);
    config_setting_t *port_setting = config_setting_add(catbox, "port", CONFIG_TYPE_INT);
    config_setting_set_int(port_setting, DEFAULT_CATPORT);
